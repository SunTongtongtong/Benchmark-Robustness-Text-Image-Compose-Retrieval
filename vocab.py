...
import json
import nltk
from config import MAIN_DIR, FASHIONIQ_ANNOTATION_DIR, SHOES_ANNOTATION_DIR, CIRR_ANNOTATION_DIR, FASHION200K_ANNOTATION_DIR, cleanCaption,CIRCO_IMAGE_DIR
...
ANNOTATIONS = {
  ...
  'circo':[f'{CIRCO_IMAGE_DIR}/annotations/{fc}.json' for fc in ['val','test']]
...
def from_circo_json(p):
  with open(p, "r") as jsonfile:
    ann = json.loads(jsonfile.read())
  captions = [cleanCaption(a["shared_concept"]) for a in ann]
  captions += [cleanCaption(a["relative_caption"]) for a in ann]
  return captions
...
def build_vocab(data_name, threshold=0):
  ...
  for p in ANNOTATIONS[data_name]:
    ...
    elif data_name == 'circo':
      captions = from_circo_json(p)
    else:
      captions = from_txt(p)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--data_name', default='fashionIQ', choices=('fashionIQ', 'shoes', 'cirr', 'fashion200K','circo'), help='Name of the dataset for which to build the vocab (fashionIQ|shoes|cirr|fashion200K)')
