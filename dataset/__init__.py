...
from .fashioniq import FashionIQDataset
from .cirr import CIRRDataset
from .transforms import get_transform

from .cirr_reason import CIRRDataset_reason
from .coco import COCODataset
from .circo import CIRCODataset
from .imgnet import imageNetRDataset
...
def get_loader_single(opt, vocab, split, transform, what_elements="triplet",
	...
	if opt.data_name == 'fashionIQ':
		dataset = FashionIQDataset(split, vocab, transform, what_elements,
					opt.load_image_feature, fashion_categories=opt.categories, img_corrupt=opt.img_corrupt , img_corrupt_level=opt.img_corrupt_level, model_name=opt.model, txt_corrupt=opt.txt_corrupt)
	...
	# 	dataset = ShoesDataset(split, vocab, transform, what_elements, opt.load_image_feature)
		...
		dataset = CIRRDataset(split, vocab, transform, what_elements, opt.load_image_feature, img_corrupt=opt.img_corrupt , img_corrupt_level=opt.img_corrupt_level, txt_corrupt=opt.txt_corrupt)
	elif opt.data_name == 'cirr_reason':
		dataset = CIRRDataset_reason(opt.reasoning_type,split, vocab, transform, what_elements, opt.load_image_feature, img_corrupt=opt.img_corrupt , img_corrupt_level=opt.img_corrupt_level, txt_corrupt=opt.txt_corrupt)
	elif opt.data_name =="coco":
		dataset = COCODataset(split, vocab, transform, what_elements, opt.load_image_feature, img_corrupt=opt.img_corrupt , img_corrupt_level=opt.img_corrupt_level, txt_corrupt=opt.txt_corrupt)
	elif opt.data_name =="imgnet":
		dataset = imageNetRDataset(split, vocab, transform, what_elements, opt.load_image_feature, img_corrupt=opt.img_corrupt , img_corrupt_level=opt.img_corrupt_level, txt_corrupt=opt.txt_corrupt)
	elif opt.data_name == 'circo':
		dataset = CIRCODataset(split, vocab, transform, what_elements, opt.load_image_feature, img_corrupt=opt.img_corrupt , img_corrupt_level=opt.img_corrupt_level, txt_corrupt=opt.txt_corrupt)
	# Data loader
	# import pdb
	# pdb.set_trace()
...
def get_eval_loader_generic(opt, vocab, split, what_elements,transform=None):
...
def get_eval_loaders(opt, vocab, split='val',transform=None):
	queries_loader = get_eval_loader_generic(opt, vocab, split, "query",transform)
	targets_loader = get_eval_loader_generic(opt, vocab, split, "target",transform)
