...
from config import CIRR_IMAGE_DIR, CIRR_ANNOTATION_DIR,IMGNET_IMAGE_DIR
...
class imageNetRDataset(BaseDataset):
    ...
    def __init__(self, split, vocab, transform, what_elements='triplet', load_image_feature=0, ** kw):
        BaseDataset.__init__(self, split, IMGNET_IMAGE_DIR, vocab, transform=transform, what_elements=what_elements, load_image_feature=load_image_feature, ** kw)      

############################################################################
        # load the paths of the images involved in the split
        query_file = osp.join(IMGNET_IMAGE_DIR, 'imgnet_real_query_ours.txt')
        target_file = osp.join(IMGNET_IMAGE_DIR, 'imgnet_targets.txt')
        ...
        self.img_query_paths = [osp.join( path.split(' ')[0]) for path in queries]              
        self.img_target_paths = [osp.join(path.split(' ')[0]) for path in targets]      
        
        self.query_label = [osp.join(path.split(' ')[1]) for path in queries]   
        self.target_label = [osp.join(path.split(' ')[1]) for path in targets]    
        cat=3
        domains = ['cartoon', 'origami', 'toy', 'sculpture']
        print('====> Domain: ', domains[cat])
        annotations_file = '/data/DERI-Gong/acw557/project/LAVIS_CVPR/LAVIS/captions/imgnet/imgnet_blip2_caption_{}.json'.format(domains[cat])
        with open(annotations_file, 'r') as f:
            self.annotations = jsonmod.load(f)
        
        # self.annotations = self.load_file(annotations_file)
    ...
    def __len__(self):
        if self.what_elements=='target':
            return len(self.img_target_paths)
        return len(self.img_query_paths)
    ...
    def get_query(self, index):
        capts = self.annotations[str(index)]
        text, real_text = self.get_transformed_captions([capts])
        print('==>',real_text)
